...
//-----------------------------//
dDensityPlot2D::dDensityPlot2D( float tPosX,    float tPosY,    float tPosZ,
...
//-----------------------------//
void dDensityPlot2D::generateBuffers() {
    if (!mGradient.empty()) {
...
void dDensityPlot2D::updateValues(const std::vector<std::vector<float>> &tNewData, size_t iNum) {
    for (size_t iRow = 0; iRow < mVert; iRow++) {
        for (size_t iColumn = 0; iColumn < mHoriz; iColumn++) {
